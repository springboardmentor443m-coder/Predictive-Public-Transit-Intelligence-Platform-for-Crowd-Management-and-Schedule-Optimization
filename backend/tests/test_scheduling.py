...
def test_handle_delay_propagation_decay(test_db):
    ...
    assert affected[0]["estimated_delay_minutes"] > 0
    assert affected[0]["estimated_eta_delay_seconds"] == int(affected[0]["estimated_delay_minutes"] * 60)

    # Hop 2 (203)
    assert affected[1]["station_code"] == "203"
    assert affected[1]["station_order"] == 2
    assert affected[1]["estimated_delay_minutes"] > 0
    assert affected[1]["estimated_eta_delay_seconds"] == int(affected[1]["estimated_delay_minutes"] * 60)

    # Hop 3 (204)
    assert affected[2]["station_code"] == "204"
    assert affected[2]["station_order"] == 3
    assert affected[2]["estimated_delay_minutes"] > 0
    assert affected[2]["estimated_eta_delay_seconds"] == int(affected[2]["estimated_delay_minutes"] * 60)
