...
from app.services.alert_engine import check_and_create_alerts, check_and_create_delay_alerts, simulate_notification
...
def test_check_and_create_alerts_and_deduplication(alert_db):
    ...
    for alert in all_alerts:
        assert alert.alert_type in ["overcrowding", "delay"]
...
def test_check_and_create_delay_alerts_explicit(alert_db):
    # Run explicit delay alert scan with lower threshold to test trigger
    delay_alerts = check_and_create_delay_alerts(alert_db, dedup_window_minutes=15, delay_threshold=0.10)
    assert isinstance(delay_alerts, list)
    if delay_alerts:
        for a in delay_alerts:
            assert a.alert_type == "delay"
            assert "delay probability" in a.message.lower()
