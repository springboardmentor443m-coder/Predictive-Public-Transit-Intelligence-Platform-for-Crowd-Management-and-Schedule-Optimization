...
def handle_delay(
    ...
    """
    Evaluates operational delay propagation across downstream stations using the trained
    Random Forest delay classifier (delay_prediction_rf_v2.pkl) and categorical encoders.
    ...
    3. Computes downstream delay propagation driven by ML delay prediction probability (predict_proba).
    ...
    # 4. Compute model-driven downstream delay propagation
    ...
    if origin_idx is not None:
        ...
        for hop, st in enumerate(next_stations, start=1):
            # Model-driven delay probability calculation
            _, delay_prob = predict_train_delay(
            ...
            # Delay propagation dynamically scaled by ML delay probability score
            # Higher delay probability preserves upstream delay; lower probability mitigates delay faster
            prob_factor = max(0.05, min(1.0, delay_prob))
            propagated_delay = round(delay_minutes * (prob_factor ** (hop * 0.5)), 1)
