...
### 2. Model Architecture: Random Forest Regressor
- **Algorithm**: Random Forest Regressor ($N_{\text{estimators}} = 50$, $\text{max\_depth} = 20$, $\text{min\_samples\_leaf} = 5$, trained on full 6,021,973-row Seoul Metro dataset).
- **Evaluation & Split**: Evaluated on a strict, chronological **time-based train/test split** (Train: 4,818,428 rows from 2015-01-01 to 2017-05-26; Held-Out Test: 1,203,545 rows from 2017-05-27 to 2018-01-01 — ~7.2 consecutive unseen future months).
- **Verified Accuracy on Held-Out Test Set**:
  - $\mathbf{R^2 = 0.9429}$ (vs. Naive station-hour baseline $R^2 = 0.8191$, representing a +0.1238 net gain)
  - $\mathbf{\text{RMSE} = 463.89\text{ passengers/hour}}$ (vs. Naive baseline $825.73$, a $43.8\%$ reduction in error)
  - $\mathbf{\text{MAE} = 190.39\text{ passengers/hour}}$ (vs. Naive baseline $414.04$, a $54.0\%$ reduction in error)
- **Verified Inference Latency**:
  - Single-Station Inference: Mean **$25.90\text{ ms}$**, Median **$23.19\text{ ms}$**, P95 **$42.85\text{ ms}$** ($38.6\text{ predictions/sec}$).
  - Vectorized Batch Inference: **$0.1855\text{ ms / station}$** amortized across 275 stations (**$5,389.9\text{ predictions/sec}$**).
...
- **Performance (Synthetic Split)**: $\text{ROC-AUC} = 0.7322$ on cross-validated training data; $0.7362$ on synthetic held-out test split.

> **Synthetic Dataset Disclosure:** Delay predictions are generated by a model trained on synthetic data (`dstevenson/synthetic_delay_dataset_2017_v2.xlsx`). This data was built using real Seoul Metro station identities and historical crowd patterns, but delay labels themselves are simulated via rule offsets ($0.15 + 0.35 \times \text{crowd} + 0.10 \times \text{peak} + \text{weather}$). This classification ROC-AUC metric measures performance on the synthetic benchmark and is separate from the real-world crowd regression model ($R^2 = 0.9429$).
...
| Metric Category | Target Goal | Verified Empirical Performance | Evaluation Note / Methodology |
| :--- | :--- | :--- | :--- |
| **Crowd Model Accuracy ($R^2$)** | $R^2 \ge 0.90$ | **$\mathbf{R^2 = 0.9429}$** | Tested on 1,203,545 held-out future rows (time-based split) |
| **Crowd Prediction Error (RMSE)** | $\text{RMSE} < 600$ | **$\mathbf{\text{RMSE} = 463.89}$** | Passengers/hour across 275 stations (MAE: $190.39$) |
| **Single-Station Latency** | $< 50.0\text{ ms}$ | **$\mathbf{25.90\text{ ms}}$** (Median: $23.19\text{ ms}$, P95: $42.85\text{ ms}$) | Measured across 1,000 single prediction iterations |
| **Batch Prediction Throughput** | $> 1,000\text{ pred/s}$ | **$\mathbf{5,389.9\text{ predictions/sec}}$** | $0.1855\text{ ms}$ amortized per station across 275-station batches |
| **API Response Time ($P_{95}$)** | $< 150.0\text{ ms}$ | **$\mathbf{145.03\text{ ms}}$** (Cached) / **$282.02\text{ ms}$** (Cold) | FastAPI async + Redis cache |
| **Prediction Cache Hit Ratio** | $> 60\%$ | **$\mathbf{63.33\%}$** | Realistic commuter workload across 131 live stations |
| **Delay Classification ROC-AUC** | $\text{ROC-AUC} \ge 0.70$ | **$\mathbf{0.7322}$** (CV) / **$\mathbf{0.7362}$** (Test) | Random Forest on synthetic delay benchmark |
| **Network Scale (Offline Training)**| $\ge 250\text{ stations}$ | **$\mathbf{275\text{ stations}}$** ($6,021,973\text{ rows}$) | Full 3-year historical dataset (2015–2018) |
| **Network Scale (Live App DB)** | $\ge 100\text{ stations}$ | **$\mathbf{131\text{ stations}}$** ($44,016\text{ rows}$) | Operationalized SQLite/Postgres DB (Lines 1–4) |
...
### 🔑 Environment-Driven Credentials (RBAC)

Initial access credentials are configured securely via environment variables in `.env` (never hardcoded or committed to git):

| Role | Default Username | Config Environment Variable | Access Level |
| :--- | :--- | :--- | :--- |
| **Station Manager (Admin)** | `admin` | `SEED_ADMIN_PASSWORD` in `.env` | Full system access, schedule override, broadcast alerts |
| **Transit Operator** | `operator` | `SEED_OPERATOR_PASSWORD` in `.env` | Real-time monitoring, alert acknowledgment, view reports |

> **Security Note:** In production, specify strong, cryptographically secure passwords in `.env` before running the database seed engine. If no password is provided in the environment, the seed engine generates ephemeral cryptographically secure random tokens.
