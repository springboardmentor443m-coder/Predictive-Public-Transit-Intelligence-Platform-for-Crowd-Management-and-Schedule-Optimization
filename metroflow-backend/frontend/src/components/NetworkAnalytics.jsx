...
export default function NetworkAnalytics() {
  ...
  const generateFallbackAnalytics = () => {
    ...
    hours.forEach(h => {
      ...
      hourlyStationMap[h.hour] = [
        { station: "Rajiv Chowk", avg_occupancy: Math.round(base * 1.12), tier: base * 1.12 >= 1500 ? "SEVERE_RUSH" : (base * 1.12 >= 800 ? "MODERATE_TRAFFIC" : "OFF_PEAK"), total_trips: 65, train_ids: [`TR_${h.hour * 100 + 10}`, `TR_${h.hour * 100 + 11}`, `TR_${h.hour * 100 + 12}`] },
        { station: "Kashmere Gate", avg_occupancy: Math.round(base * 1.06), tier: base * 1.06 >= 1500 ? "SEVERE_RUSH" : (base * 1.06 >= 800 ? "MODERATE_TRAFFIC" : "OFF_PEAK"), total_trips: 62, train_ids: [`TR_${h.hour * 100 + 20}`, `TR_${h.hour * 100 + 21}`, `TR_${h.hour * 100 + 22}`] },
        { station: "Hauz Khas", avg_occupancy: Math.round(base * 0.98), tier: base * 0.98 >= 1500 ? "SEVERE_RUSH" : (base * 0.98 >= 800 ? "MODERATE_TRAFFIC" : "OFF_PEAK"), total_trips: 58, train_ids: [`TR_${h.hour * 100 + 30}`, `TR_${h.hour * 100 + 31}`, `TR_${h.hour * 100 + 32}`] },
        { station: "Botanical Garden", avg_occupancy: Math.round(base * 0.88), tier: base * 0.88 >= 1500 ? "SEVERE_RUSH" : (base * 0.88 >= 800 ? "MODERATE_TRAFFIC" : "OFF_PEAK"), total_trips: 52, train_ids: [`TR_${h.hour * 100 + 40}`, `TR_${h.hour * 100 + 41}`, `TR_${h.hour * 100 + 42}`] },
        { station: "Dwarka Sec 21", avg_occupancy: Math.round(base * 0.78), tier: base * 0.78 >= 1500 ? "SEVERE_RUSH" : (base * 0.78 >= 800 ? "MODERATE_TRAFFIC" : "OFF_PEAK"), total_trips: 48, train_ids: [`TR_${h.hour * 100 + 50}`, `TR_${h.hour * 100 + 51}`, `TR_${h.hour * 100 + 52}`] }
  ...
  const hourlyStationRanks = analytics?.hourly_station_breakdown?.[activeHourInt] || [
    { station: "Rajiv Chowk", avg_occupancy: Math.round((activeHourData?.avg_occupancy || 1700) * 1.12), tier: "SEVERE_RUSH", train_ids: ["TR_7977", "TR_3748", "TR_6561"] },
    { station: "Kashmere Gate", avg_occupancy: Math.round((activeHourData?.avg_occupancy || 1700) * 1.06), tier: "SEVERE_RUSH", train_ids: ["TR_9528", "TR_1412", "TR_8506"] },
    { station: "Hauz Khas", avg_occupancy: Math.round((activeHourData?.avg_occupancy || 1700) * 0.98), tier: "SEVERE_RUSH", train_ids: ["TR_4144", "TR_8516", "TR_6831"] },
    { station: "Botanical Garden", avg_occupancy: Math.round((activeHourData?.avg_occupancy || 1700) * 0.88), tier: "MODERATE_TRAFFIC", train_ids: ["TR_3887", "TR_1246", "TR_2453"] },
    { station: "Dwarka Sec 21", avg_occupancy: Math.round((activeHourData?.avg_occupancy || 1700) * 0.78), tier: "MODERATE_TRAFFIC", train_ids: ["TR_2709", "TR_5930", "TR_3173"] }
  ];
    ...
    <div className="space-y-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <StatCard
          ...
          value="89.9%"
          subtitle="Predictions land within ~10% of actual counts on average across all 5,001 trips — a high-confidence model."
          icon={TrendingUp}
          color="emerald"
        ...
        <StatCard
          ...
          value="87.3%"
          subtitle="Even during rush-hour surges, worst-case deviations stay within ~13% of actual crowd levels."
          icon={Activity}
          color="purple"
          badge="Peak-Safe"
        />
              ...
              <div className="space-y-3 mt-4">
                {hourlyStationRanks.map((st, idx) => {
                  ...
                  return (
                    <div 
                      key={idx}
                      className="p-3.5 rounded-2xl bg-slate-950/70 border border-slate-800/90 flex flex-col sm:flex-row sm:items-center justify-between gap-3 transition-all hover:border-slate-700 hover:bg-slate-950/90"
                    >
                      <div className="flex items-start space-x-3">
                        <span className={`w-6 h-6 rounded-lg text-xs font-mono font-bold flex items-center justify-center border flex-shrink-0 mt-0.5 ${
                          idx === 0 
                        ...
                        <div className="space-y-1">
                          ...
                          {/* Visible Train IDs directly below station */}
                          <div className="flex flex-wrap items-center gap-1.5 pt-1">
                            ...
                            {st.train_ids && st.train_ids.length > 0 ? (
                              st.train_ids.map((tid, tIdx) => (
                                <span 
                                  key={tIdx} 
                                  className="px-1.5 py-0.5 text-[10px] font-mono font-bold bg-slate-900 text-cyan-300 rounded border border-cyan-500/30 shadow-inner"
                                >
                                  {tid}
                                </span>
                              ))
                            ) : (
                              <span className="px-1.5 py-0.5 text-[10px] font-mono text-slate-400 bg-slate-900 rounded border border-slate-800">
                                TR_{(activeHourInt || 18) * 100 + (idx + 1) * 10}
                              </span>
                            )}
                          </div>
                        </div>
                      ...
                      <div className="text-right font-mono flex-shrink-0 sm:self-center">
