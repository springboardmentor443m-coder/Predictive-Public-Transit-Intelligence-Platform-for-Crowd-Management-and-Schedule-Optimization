...
export default function ScheduleAdvisoryTable({ predictionParams }) {
  ...
  // Auto-load schedule when prediction params change
  useEffect(() => {
    if (predictionParams) {
      loadData(predictionParams);
    }
  }, [predictionParams]);
  ...
  const loadData = async (overrideParams) => {
    ...
    try {
      const params = overrideParams || predictionParams;
      ...
      console.error("Failed to load schedule advisory:", err);
      generateFallbackDirectives();
      setRefreshSuccess(true);
      setTimeout(() => setRefreshSuccess(false), 2500);
    } finally {
      ...
      <div className="glass-panel rounded-3xl border border-slate-800 overflow-hidden shadow-2xl">
        ...
        <div className="p-6 border-b border-slate-800 bg-slate-900/60 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          ...
          <div className="flex items-center flex-wrap gap-3">
            ...
            {/* Prediction-Based Indicator */}
            {predictionParams && (
              <div className="flex items-center gap-1.5 px-3 py-2 rounded-2xl bg-purple-500/15 text-purple-300 border border-purple-500/30 text-xs font-mono">
                <Zap className="w-3.5 h-3.5" />
                <span>AI-Powered: {predictionParams.from_station} → {predictionParams.to_station}</span>
              </div>
            )}
