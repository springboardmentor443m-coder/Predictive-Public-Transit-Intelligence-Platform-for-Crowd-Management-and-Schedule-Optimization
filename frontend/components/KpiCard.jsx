...
export default function KpiCard({ label, value, change, trend = "neutral", icon: Icon, accent = "brand", sub }) {
  const accents = {
  ...
  return (
    <div className={`card card-pad card-hover flex items-center gap-4 border-slate-800 ${glows[accent] || glows.brand}`}>
      {Icon && (
        <span className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl border ${accents[accent] || accents.brand}`}>
